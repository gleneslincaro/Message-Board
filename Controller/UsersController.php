	...
	public function add(){
		...
		if ($this->request->is('post')){
			...
			if ($this->User->save($this->request->data)){
				...
				$this->Flash->set('User was added.');
				//redirect to user list
				$this->redirect(array('action' => 'index'));
				 
			}else{
				//if save failed
				$this->Flash->set('Unable to add user. Please, try again.');
		...
		if( $this->User->exists() ){
			...
			if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
				...
				if( $this->User->save( $this->request->data ) ){
					...
					$this->Flash->set('User was edited.');
					 
					//redirect to user's list
					$this->redirect(array('action' => 'index'));
					 
				}else{
					$this->Flash->set('Unable to edit user. Please, try again.');
				}
			...
			$this->Flash->set('The user you are trying to edit does not exist.');
			$this->redirect(array('action' => 'index'));
		...
		if( $this->request->is('get') ){
			...
			if( !$id ) {
				...
				if( $this->User->delete( $id ) ){
					//set to screen
					$this->Flash->set('User was deleted.');
					//redirect to users's list
					$this->redirect(array('action'=>'index'));
					 
				}else{  
					//if unable to delete
					$this->Flash->set('Unable to delete user.');
					$this->redirect(array('action' => 'index'));
				}
			}
		}
